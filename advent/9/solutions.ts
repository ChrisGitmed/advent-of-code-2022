// Part 1
export const getCountOfVisitedPositionsOne = (input:string) => {
  ...
  for (const movement of steps) {
    ...
    for (let i = 0; i < +moveCount; i++) {
      ...
      for (let j = 1; j < ropeLength; j++) {
        ...
        if (distance.every((coordinate) => Math.abs(coordinate) <= 1)) continue;
...
// Part 2
export const getCountOfVisitedPositionsTwo = (input:string) => {  
  const steps = input.split('\n');
